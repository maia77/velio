...
"""
سكريبت لحذف منتجات زينه الطبيعه الثابته من قاعدة البيانات
"""
...
def find_fixed_nature_products(conn, db_type):
    """البحث عن منتجات زينه الطبيعه الثابته"""
    try:
        cursor = conn.cursor()
        ...
        if db_type == 'sqlite':
            ...
            FROM products 
            WHERE main_category = 'زينه الطبيعه الثابته' 
               OR main_category_ar = 'زينه الطبيعه الثابته'
               OR category = 'زينه الطبيعه الثابته'
               OR category_ar = 'زينه الطبيعه الثابته'
               OR main_category = 'زينة الطبيعة الثابتة' 
               OR main_category_ar = 'زينة الطبيعة الثابتة'
               OR category = 'زينة الطبيعة الثابتة'
               OR category_ar = 'زينة الطبيعة الثابتة'
            """
        else:
            # البحث في PostgreSQL
            query = """
        ...
        print(f"🌿 تم العثور على {len(products)} منتج في قسم زينه الطبيعه الثابته:")
        print("-" * 80)
...
def delete_fixed_nature_products(conn, db_type, products):
    """حذف منتجات زينه الطبيعه الثابته"""
    if not products:
...
def main():
    """الدالة الرئيسية"""
    print("🌿 سكريبت حذف منتجات زينه الطبيعه الثابته")
    print("=" * 50)
    ...
    try:
        # البحث عن المنتجات
        products = find_fixed_nature_products(conn, db_type)
        
        if not products:
            print("ℹ️ لا توجد منتجات في قسم زينه الطبيعه الثابته")
            return
        
        # تأكيد الحذف
        print(f"\n⚠️ هل تريد حذف {len(products)} منتج من قسم زينه الطبيعه الثابته؟")
        confirm = input("اكتب 'نعم' للتأكيد: ").strip()
        
        if confirm.lower() in ['نعم', 'yes', 'y']:
            deleted_count = delete_fixed_nature_products(conn, db_type, products)
