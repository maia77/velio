...
# إعداد قاعدة البيانات بمسار مطلق آمن وإنشاء مجلد instance تلقائيًا
...
with app.app_context():
    try:
        ...
        # التحقق من وجود الحقول الجديدة وإضافتها إذا لزم الأمر
        try:
            ...
            if test_product:
                ...
                if not hasattr(test_product, 'main_category'):
                    ...
                    db.engine.execute("ALTER TABLE products ADD COLUMN main_category TEXT DEFAULT 'أصالة معاصرة'")
                    db.engine.execute("ALTER TABLE products ADD COLUMN main_category_ar TEXT DEFAULT 'أصالة معاصرة'")
                    db.engine.execute("ALTER TABLE products ADD COLUMN is_home_essentials BOOLEAN DEFAULT 1")
                    db.engine.execute("ALTER TABLE products ADD COLUMN is_new_arrival BOOLEAN DEFAULT 0")
                    
                    # تحديث المنتجات الموجودة
                    db.engine.execute("UPDATE products SET main_category = 'أصالة معاصرة', main_category_ar = 'أصالة معاصرة' WHERE main_category IS NULL")
