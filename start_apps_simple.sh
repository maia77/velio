#!/bin/bash

# ملف تشغيل مبسط للتطبيقين

echo "🚀 بدء تشغيل التطبيقين مع قاعدة البيانات المشتركة"
echo "============================================================"

# التحقق من وجود Python
if ! command -v python3 &> /dev/null; then
    echo "❌ Python3 غير مثبت. يرجى تثبيت Python3 أولاً"
    exit 1
fi

echo "✅ تم التحقق من Python3"

# إعداد قاعدة البيانات
echo "🔧 إعداد قاعدة البيانات..."
python3 simple_setup.py

# اختبار الاتصال بقاعدة البيانات
echo "🔍 اختبار الاتصال بقاعدة البيانات..."
python3 shared_database_config_fallback.py

echo ""
echo "📋 معلومات التشغيل:"
echo "   🌐 تطبيق الويب: http://127.0.0.1:5003"
echo "   🔧 تطبيق الإدارة: http://127.0.0.1:5007"
echo "   💾 قاعدة البيانات: مشتركة بين التطبيقين (SQLite محلية)"
echo ""

# تشغيل التطبيقين في خلفية منفصلة
echo "🚀 بدء تشغيل التطبيقين..."

# تشغيل تطبيق الويب في الخلفية
cd web
python3 app.py &
WEB_PID=$!

# العودة للمجلد الرئيسي
cd ..

# تشغيل تطبيق الإدارة في الخلفية
cd admin-app
python3 admin_app_fixed.py &
ADMIN_PID=$!

# العودة للمجلد الرئيسي
cd ..

echo "✅ تم بدء تشغيل التطبيقين بنجاح!"
echo "🔄 اضغط Ctrl+C لإيقاف التطبيقين"

# انتظار إشارة الإيقاف
trap "echo '🛑 إيقاف التطبيقين...'; kill $WEB_PID $ADMIN_PID; exit" INT

# انتظار انتهاء العمليات
wait
